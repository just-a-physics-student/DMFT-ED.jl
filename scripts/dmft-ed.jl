...
# example call: rm Desktop/rm_me_fork/* ; julia +1.11 .julia/dev/DMFT-ED.jl/scripts/dmft-ed.jl 4 1.0 2.0 3.0 1.0 "2Dsc-0.25" 20 4 /home/jan/Desktop/rm_me_fork/ --checkpoint --n_frequencies 1000
using ArgParse
...
function parse_commandline()
    ...
    @add_arg_table s begin
        ...
        "--resume"
            ...
            action = :store_true
        "--n_frequencies"
            help = "Number of positive Matsubara frequencies to be used."
            arg_type = Int
            default = 1000
        "--convergence_parameter"
            help = "Threshold that if exceeded leads to convergency. The quantity which is checked is documented under `abs_conv` in the function `DMFT_Loop`."
            arg_type = Float64
            default = 1e-7
        "scan_mode"
        ...
        "n_bath_sites"
            ...
            arg_type = Int
            required = true
        "out_dir" # change to output file in future!
...
for hubbard_u in hubbard_u_values
    ...
    for inverse_temperature in inverse_temperature_values
        ...
        @time anderson_parameters, GF_imp, Σ_imp, partition_sum, E_min, double_occupancy, density, converged, νnGrid, convergence_parameter_out = DMFT_Loop(
            hubbard_u, chemical_potential, inverse_temperature,
            anderson_parameters, lattice_info, Nk = bz_points_per_dim, Nν=n_frequencies,
            abs_conv=convergence_parameter, maxit=max_iterations, checkpointfile=checkpointfile)
        ...
        write_result(out_file_path, hubbard_u, inverse_temperature, chemical_potential, anderson_parameters, partition_sum, GF_imp.parent, Σ_imp.parent,
            density, double_occupancy, E_min, n_bath_sites, lattice_info, converged, bz_points_per_dim, convergence_parameter_out, scan_mode)
